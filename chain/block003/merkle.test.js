...
describe("테스트 들의 묶음 단위 내용", () => {
  // 각각의 테스트들을 여기에 작성하면 된다.
  // 테스트 단위
  it("테스트의 내용1", () => {
    console.log("나 처음써봐요");
  });
  ...
  it("테스트의 내용5", () => {
    expect("a").toBe("a");
    // expect("a").toBe("b");
    // expect() 함수는 비교 함수들을 사용할 수 있게 해준다.
    // expect() 사용방법은 매개변수로 비교할 값을 넣어주고
    // expect().toBe()의 toBe() 매개변수로 앞의 값과 비교할 값을 넣어준다.
    // 단순히 데이터비교 A와 B를 넣었으면 A===B 인지 확인을 한다.
    // 만약 같으면 통과  expect("a").toBe("a");
    // 만약 다르면 불통과  expect("a").toBe("b");
  });
});
// describe 함수 자체가 테스트의 묶음으로 그룹화 해줄 수 있다.
