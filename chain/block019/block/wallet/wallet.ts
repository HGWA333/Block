...
class Wallet implements IWallet {
  ...
  constructor(_privateKey: string = "") {
    console.log("2-3(지갑이 없을 때)/4-4(지갑이 있을 때) 지갑 생성 시작");
    ...
    console.log(
      "2-6()/4-7() 지갑 주소 이름으로 파일 생성 후 그 내용으로 개인키 저장"
    );
    // address가 파일명이 됨
  ...
  public getPublickKey(): string {
    // 공개키를 가져오는 용도
    console.log(
      "2-4(개인키 있을 때 공개키 생성하는 상황)/4-5() 생성된 개인키로 공개키 생성"
    );
    return ec
  ...
  static getWalletPrivateKey(_address) {
    ...
    console.log(
      "4-3/5-5 지갑 주소를 파일 명으로 생성한 파일을 불러와서 그 내용의 개인키를 가져온다."
    );
