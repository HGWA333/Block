...
class Wallet implements IWallet {
  ...
  constructor(_privateKey: string = "") {
    ...
    // address가 파일명이 됨
    // 2-4, 4-7
    !fs.existsSync(addresDir) && fs.mkdirSync(addresDir);
  ...
  public getAddress(): string {
    // 지갑 주소 만드는 용도
    console.log(
      "2-6(지갑이 없어 주소를 새로 생성하는 상황)/4-6(지갑이 있고 주소 목록이 있는 상황) 공개키로 지갑 주소 생성"
    );
  ...
  static getWalletPrivateKey(_address) {
    ...
    console.log(
      "4-3(지갑이 있고 주소 목록이 있는 상황)/5-5(지갑이 있고 주소 목록이 있는 상황)/6-5 지갑 주소를 파일 명으로 생성한 파일을 불러와서 그 내용의 개인키를 가져온다."
    );
