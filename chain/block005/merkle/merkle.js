...
// merkle('sha256)은 merkle 라이브러리를 사용한다. 매개변수로는 암호화 방식을 전달한다.

// .sync(data) merkle Tree를 만들 데이터를 매개변수로 전달한다.

// .root() 완성된 merkle Tree의 Root(완성된 하나의 암호화된 데이터)
// merkle 라이브러리는 받는 데이터 배열의 아이템을 전부 문자열(string)으로 처리 한다.

const createMerkleRoot = () => {
  return SHA256(`${data[0]}`).toString().toUpperCase();
  //   return SHA256(`${data[0]}`).toString().toUpperCase();
  //   return SHA256(data[0].toString()).toString().toUpperCase();
  //   위 2개와 같이  숫자로 된 것을 문자열로 바꿔준다.

  // SHA256(data[0]) 는 데이터를 암호화 하고 암호화된 데이터는 객체 형식이다.
  // .toString() 는 문자열로 변환한다.
  // .toUpperCase() 는 SHA256을 사용할 경우 소문자로 나타나기 때문에 대문자로 수정해준다.
};
