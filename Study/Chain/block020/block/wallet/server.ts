...
app.post("/transaction/send", (req: Request, res: Response) => {
  ...
  console.log(
    "5-7 생성한 서명과 hash를 만들기 위한 데이터를 가지고, http://localhost:8080/transaction/send에 요청을 보냄 검증 과정"
  );
...
app.post("/balance", () => { axios.post("http://localhost:8080/transaction/send", txObj, {
  headers: {
    Authorization: "Basic " + Buffer.from("admin:1234").toString("base64"),
    // Authorization는 보안 인증 설정
    // HTTP 통신에서의 인증 방법
