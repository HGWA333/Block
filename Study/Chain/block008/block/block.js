...
class BlockHeader {
  ...
  getDifficulty({
    _previousDifficulty, // 이전 블록 난이도
    _adjustmentDifficulty, // 난이도 조절 단위 개수 이전 블록의 난이도
    _adjustmentTimestamp, // 난이도 조절 단위 개수 이전 블록의 생성 시간
    _DAI, // 난이도 조절 단위 갯수
    _AGtime, // 블록 세대당 생성 시간, 블록 10개당 생성 시간
  }) {
    if (this.height < _DAI) {
      this.difficulty = 0;
    }
    // 10개 이전에는 제네시스 블록 생성 시 설정한 난이도(0)으로 설정(정의)
    else if (this.height < _DAI * 2) {
      this.difficulty = 1;
    }
    // 20개 이전에는 제네시스 블록 생성 시 설정한 난이도보다 하나 더 높은 난이도가 설정된다.
    else if (this.height % _DAI !== 0) {
      // 높이가 난이도 조절 단위 갯수에 맞지 않을 때
      this.difficulty = _previousDifficulty;
    } // 20개 이전에는 제네시스 블록 생성 시 설정한 난이도보다 하나 더 높은 난이도가 설정된다.
    else {
      const timeToken = this.timestamp - _adjustmentTimestamp;
      ...
      console.log("10개 전 블록 생성 시간:", _AGtime);

      if (timeToken < _AGtime * 0.5) {
        // 이전 10개 생성 시간이 5분보다 적게 걸렸을 때
        this.difficulty = _adjustmentDifficulty + 1;
        // 난이도를 올려서 시간이 더 걸릴 수 있게 조절한다.
      } else if (timeToken < _AGtime * 1.5) {
        // 이전 10개 생성 시간이 5분보다 적게 걸렸을 때
        this.difficulty = _adjustmentDifficulty - 1;
        // 난이도를 낮춰서 시간이 덜 걸릴 수 있게 조절한다.
      } else {
        this.difficulty = _adjustmentDifficulty;
      }
    }
  }
}
  ...
  constructor(_data, _previousBlock, _adjusetmentblock, _config) {
    ...
    if (this.merkelRoot) {
      ...
      if (_adjusetmentblock && _config) {
        this.getDifficulty({
          previousDifficulty: _previousBlock.difficulty, // 이전 블록 난이도
          adjustmentDifficulty: _adjusetmentblock.difficulty, // 난이도 조절 단위 개수 이전 블록의 난이도
          adjustmentTimestamp: _adjusetmentblock.timestamp, // 난이도 조절 단위 개수 이전 블록의 생성 시간
          DAI: _config.DAI, // 난이도 조절 단위 갯수
